...
def box_plot_analyse(df: pd.DataFrame, to_show: bool):
    marital_status = [
            "Single",
            "Married",
            "Widower",
    ...
    for feature in features_to_plot:
        ...
        sns.boxplot(data=df, x="Target", y=feature, ax=ax_single)
        ax_single.set_title(f"{feature} by Target")
        add_median_to_plot_by_cat(df, feature, "Target", ax_single)
        plots.append(fig_single)

    fig_single, ax_single = plt.subplots(figsize=(6, 5))
    sns.boxplot(data=df, x="Marital status", y="Age at enrollment", ax=ax_single)
